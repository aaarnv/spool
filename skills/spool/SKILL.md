...
--pr <number|url>` composes the PR body from `change.json`: the request as the opening
paragraph, Changes (one bullet per outcome, with a deep link), Before / after (the pairs),
Testing (the detail of each test evidence item), Deviations, Not verified (unknowns), then
one line linking the walkthrough. No model reads the diff. Write the record for a reader:
short claims, test details that say what ran. Whatever a person already wrote in the body is kept, collapsed, inside the fenced
...
**The GitHub App already seeds automatically.** Installing the Spool App on a repository starts
its project and, for repositories with recaps on, builds the knowledge base from the README, the
docs and the file tree without anyone running a command. `spool init` is the MANUAL path, and it
is still the better one: the App reads the repository through the GitHub API and never boots
anything, so it authors no `set_recording` topics beyond what the docs state outright. Run
`spool init` when the project needs the dev command, the port, the sign-in shape or which flows
demo well. It merges with what the App wrote rather than replacing it, and `knowledge.json` shows
...
plan. Read them, act on them, reply.

```bash
spool read <workdir-or-share-dir>      # prints "open comments" with the time and the id
```

Through MCP:

- `list_comments(spoolId)` — every comment that is not closed, with `[mm:ss–mm:ss]`, who
  wrote it, and the id.
...
comment is where somebody says the narration is wrong or the take missed the point, and
fixing that is usually a re-record, not an argument.
