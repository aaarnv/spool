...
export const DIAG_W = 480, DIAG_H = 260;
...
export function resolveLabels(shapes, measure) {
  ...
  for (const sh of all) {
    const r = rectOf(sh);
    if (r) { rects.set(sh.id, r); if (AVOID.has(sh.type)) panels.push(r); }
    const L = labelOf(sh);
    if (!L) continue;
    fit(L, avail);
    L.base = L.size;
  ...
  const clearOf = (L, host) => !panels.some((r) => r !== host && !holds(r, host) && overRect(L, r));
  ...
  const placed = [];
  const seat = (L, r, name) => { fit(L, room(r, name)); return place(L, r, name); };
  ...
  const settle = (L) => {
    ...
    boxOf(L);
    const home = L.y;
    if (panels.some((r) => straddles(L, r))) {
      ...
      const ok = spots
        ...
        .filter((c) => { L.x = c.x; L.y = c.y; boxOf(L); return !panels.some((r) => straddles(L, r)); });
  ...
  const slotsFor = (L) => (L.person ? ['above', 'below'] : ['above', 'insideTop', 'insideBottom', 'below']);
  ...
  for (const L of above) {
    ...
    for (const o of row) {
      ...
      if (!clearOf(o, o.host) || o.top < SAFE || clash) { o.y = back; boxOf(o); }
    }
  }
  ...
  for (const [id, group] of groups) {
    ...
    for (const L of group) fit(L, wide);
    const size = Math.min(...group.map((L) => L.size));
    for (const L of group) { L.size = size; fit(L, wide); }
  ...
  // 5. a single note takes the slot nearest where it was anchored, staying with
  //    its panel's interior before it drifts to the far side
  const order = (L, r) => {
    if (L.ay < r.y) return ['above', 'insideTop', 'insideBottom', 'below'];
    if (L.ay > r.bot) return ['below', 'insideBottom', 'insideTop', 'above'];
    return L.ay < r.cy ? ['insideTop', 'insideBottom', 'above', 'below']
      : ['insideBottom', 'insideTop', 'below', 'above'];
  };
  for (const L of list) if (L.host && !L.done && L.kind === 'note') firstSlot(L, order(L, L.host));
