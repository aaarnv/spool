...
export function validateChange(doc, { stepNames = null } = {}) {
  ...
  const checkStep = (value, path) => {
    ...
    if (steps && !steps.has(value.trim())) {
      error(path, 'unknown-step', `this spool has no step "${value.trim()}". Anchor to one of: ${[...steps].join(', ') || '(none)'}`);
    }
  };
    ...
    if (!isObject(doc.intent)) {
      ...
      if (interpretation !== undefined && interpretation !== null) {
        if (!isObject(interpretation)) {
          ...
          if (constraints !== undefined && constraints !== null && !Array.isArray(constraints)) {
            error('intent.interpretation.constraints', 'invalid-type', 'constraints must be an array of strings');
      ...
      if (clarifications !== undefined && clarifications !== null && !Array.isArray(clarifications)) {
        ...
        items.forEach((c, i) => {
          ...
          if (!isObject(c)) return error(at, 'invalid-type', 'each clarification must be an object');
          const id = text(c.id);
          if (!id) error(`${at}.id`, 'required', 'each clarification needs an id');
          else if (seen.has(id)) error(`${at}.id`, 'duplicate', `clarification id "${id}" is used twice`);
          checkText(c.text, `${at}.text`);
          if (!STATEMENT_SOURCES.includes(c.source)) {
            error(`${at}.source`, 'invalid-source', `source must be one of ${STATEMENT_SOURCES.join(', ')}`);
          }
          checkAt(c.at, `${at}.at`);
          ...
          if (c.supersedes !== undefined && c.supersedes !== null) {
            ...
            if (!seen.has(target)) {
              error(`${at}.supersedes`, 'unknown-clarification', `supersedes must name an EARLIER clarification id (got ${JSON.stringify(c.supersedes)})`);
            }
          }
          if (id) seen.add(id);
        });
  ...
  if (doc.evidence !== undefined && doc.evidence !== null && !Array.isArray(doc.evidence)) {
    ...
    items.forEach((e, i) => {
      const at = `evidence[${i}]`;
      if (!isObject(e)) return error(at, 'invalid-type', 'each evidence item must be an object');
      const id = text(e.id);
      if (!id) error(`${at}.id`, 'required', 'each evidence item needs an id');
      else if (evidenceIds.has(id)) error(`${at}.id`, 'duplicate', `evidence id "${id}" is used twice`);
      else evidenceIds.add(id);
      if (!EVIDENCE_TYPES.includes(e.type)) {
        error(`${at}.type`, 'invalid-type-value', `type must be one of ${EVIDENCE_TYPES.join(', ')}`);
      }
      ...
      if (e.type === 'test' && !isText(e.detail)) {
        error(`${at}.detail`, 'required', 'test evidence must say what ran and its scope, for example "tsc --noEmit and next build, web/ only"');
      }
    });
  }
    ...
    if (!isObject(doc.result)) {
      ...
      if (doc.result.outcomes !== undefined && doc.result.outcomes !== null && !Array.isArray(doc.result.outcomes)) {
        ...
        items.forEach((o, i) => {
          ...
          if (!isObject(o)) return error(at, 'invalid-type', 'each outcome must be an object');
          const id = text(o.id);
          if (!id) error(`${at}.id`, 'required', 'each outcome needs an id');
          else if (outcomeIds.has(id)) error(`${at}.id`, 'duplicate', `outcome id "${id}" is used twice`);
          else outcomeIds.add(id);
          checkText(o.claim, `${at}.claim`);
          if (!OUTCOME_STATUSES.includes(o.status)) {
            error(`${at}.status`, 'invalid-status', `status must be one of ${OUTCOME_STATUSES.join(', ')}`);
          }
          ...
          if (cited !== undefined && cited !== null && !Array.isArray(cited)) {
            error(`${at}.evidence`, 'invalid-type', 'evidence must be an array of evidence ids');
          } else {
            list(cited).forEach((ref, j) => {
              const key = text(ref);
              if (!key) return error(`${at}.evidence[${j}]`, 'invalid-type', 'an evidence citation must be an id string');
              if (!evidenceIds.has(key)) error(`${at}.evidence[${j}]`, 'unknown-evidence', `no evidence item has id "${key}"`);
            });
          }
      ...
      if (doc.result.deviations !== undefined && doc.result.deviations !== null && !Array.isArray(doc.result.deviations)) {
        ...
        items.forEach((d, i) => {
          ...
          if (!isObject(d)) return error(at, 'invalid-type', 'each deviation must be an object');
          checkText(d.note, `${at}.note`);
          if (d.outcome !== undefined && d.outcome !== null) {
            const key = text(d.outcome);
            if (!outcomeIds.has(key)) error(`${at}.outcome`, 'unknown-outcome', `no outcome has id "${key}"`);
          }
        });
      }
      ...
      if (doc.result.unknowns !== undefined && doc.result.unknowns !== null && !Array.isArray(doc.result.unknowns)) {
        ...
        list(doc.result.unknowns).forEach((u, i) => checkText(u, `result.unknowns[${i}]`));
      }
    }
  }
