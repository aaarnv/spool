...
- **OpenRouter (default)** — `OPENROUTER_API_KEY` (env, the project's `.env`, or `openrouterKey`
  ...
  `SPOOL_TTS_VOICE` (Flux voices are `flux-<name>-en`, default `flux-drew-en`); or
...
or `SPOOL_ENGINE` for a single run. Word timings stay on whisper: your own `OPENAI_API_KEY` when
set, else `openai/whisper-1` through OpenRouter.
