...
import { BLOB_BASE, blobUrl, type Loom } from "../../loom";
...
// the authoritative loom.json/transcript/console itself, then hands the CLI scoped,
// short-lived client tokens to PUT the big files straight to Blob storage. Blob URLs
// are deterministic (addRandomSuffix:false), so we can rewrite loom.json up front.
const MAX_STEPS = 200;
...
type Body = { loom: Loom; transcript?: string; console?: string };

export async function POST(req: Request) {
  const token = process.env.LOOM_PUBLISH_TOKEN;
  const auth = req.headers.get("authorization") || "";
  if (!token || auth !== `Bearer ${token}`) return bad(401, "unauthorized");
  if (!BLOB_BASE) return bad(500, "LOOM_BLOB_BASE not configured");
  ...
  const loom = body?.loom;
  if (!loom || !Array.isArray(loom.steps)) return bad(400, "missing loom");
  if (loom.steps.length > MAX_STEPS) return bad(413, "too many steps");
  ...
  loom.video = blobUrl(id, "final.mp4");
  ...
  await Promise.all([
    write("loom.json", JSON.stringify(loom, null, 2), "application/json"),
